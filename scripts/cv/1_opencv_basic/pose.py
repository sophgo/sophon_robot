  ...
  def callback(self,data):
    if self.i % 2 == 0:
      ...
      if ret == True:
        ...
        self.cv_image = cv.drawContours(self.cv_image, [imgpts[:4]],-1,(0,255,0),-3)
        # draw pillars in blue color
        for i,j in zip(range(4),range(4,8)):
            self.cv_image = cv.line(self.cv_image, tuple(imgpts[i]), tuple(imgpts[j]),(255),3)
        # draw top layer in red color
        self.cv_image = cv.drawContours(self.cv_image, [imgpts[4:]],-1,(0,0,255),3)
    #cv.imshow('3D Image window',cv_image)
    #cv.waitKey(3)
    self.i += 1
  
    try:
      self.image_pub.publish(self.bridge.cv2_to_compressed_imgmsg(self.cv_image))
    except CvBridgeError as e:
      print(e)
