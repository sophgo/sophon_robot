...
from sophon_robot.cfg import LineFollowConfig
...
class image_converter:
  ...
  def __init__(self):
    ...
    self.topic_name = rospy.get_param("~topic_name","line_following")
  ...
  def callback(self,data):
    ...
    if self.setcolor:
      ...
      if len(contours) != 0:
        ...
        cv.line(cv_image,(int(width/2),height),(x,y),(255,0,0),2)

        err = (width/2 - x)/float((height - y))
        #print(err)
        self.cmd.angular.z = (self.kp*err + self.kd*(err - self.last_err))*1
        #print(self.cmd)
