...
class image_converter:
  ...
  def __init__(self):
    ...
    self.image_pub_del = rospy.Publisher(self.topic_name+"/DelateImage/compressed", CompressedImage,queue_size=10)
  ...
  def callback(self,data):
    ...
    frameDelta = cv.cvtColor(thresh, cv.COLOR_GRAY2BGR)
    # import pdb;pdb.set_trace()
    try:
      self.image_pub.publish(self.bridge.cv2_to_compressed_imgmsg(frame))
      self.image_pub_del.publish(self.bridge.cv2_to_compressed_imgmsg(frameDelta))
    except CvBridgeError as e:
      print(e)
